...
class DrQV2Agent:
    ...
    def update_critic(self, obs, action, reward, discount, next_obs, step, obs_original):
        ...
        if self.tangent_prop:
            ...
            # diffeo aug
            # diffeo = data_augmentation.Diffeo()
            diffeo = kornia.augmentation.RandomElasticTransform(kernel_size=(31, 31), p=1., same_on_batch=False, keepdim=True)
            diff_aug = diffeo(obs_original)
            # calculate diff critic loss
            Q1, Q2 = self.critic(self.encoder(diff_aug), action)
            #     huber loss
            # diff_critic_loss = F.mse_loss(Q1, avg_target_Q) + F.mse_loss(Q2, avg_target_Q)
            batch_size = Q1.size(0)
            diff_mse_loss = torch.square(Q1 - avg_target_Q) + torch.square(Q2 - avg_target_Q)
            mask = diff_mse_loss < avg_critic_loss
            diff_critic_loss = torch.sum(diff_mse_loss[mask]) / batch_size
            # print(type(Q1))
            # print(Q1.shape)
            # print(diff_critic_loss, avg_critic_loss)
            # print(F.mse_loss(Q1, avg_target_Q) + F.mse_loss(Q2, avg_target_Q), torch.sum(diff_mse_loss)/batch_size)
            # if diff_critic_loss < avg_critic_loss:
            #     avg_critic_loss += diff_critic_loss
            avg_critic_loss += diff_critic_loss
    ...
    def update(self, replay_iter, step):
        ...
        for k in range(self.aug_K):
            ...
            # for i in range(len(obs.float())):
            #     pic = obs.float()[i]
            #     pic_numpy = pic.cpu().numpy()[:3, :, :]
            #     pic_numpy = np.transpose(pic_numpy, (1,2,0))
            #     # print(pic_numpy.shape)
            #     # print(pic_numpy.dtype)
            #     # print(pic_numpy)
            #     plt.imshow(pic_numpy)
            #     plt.axis('off') 
            #     plt.savefig(f'/bigdata/users/ve490-fall23/lyf/drqv2/records/input_{i}.png')
            #     plt.close()
            #     # image = Image.fromarray(pic_numpy)  # Transpose dimensions if needed
            #     # image.save(f'/bigdata/users/ve490-fall23/lyf/drqv2/records/saliency_{i}.png')  # Save the image with a unique name
            # raise NotImplementedError
            next_obs_aug = self.aug(next_obs.float())
            # obs_aug = obs.float()
            # next_obs_aug = next_obs.float()
            # encoder
            obs_all.append(self.encoder(obs_aug))
            # obs_all.append(self.feature_aug(self.encoder(obs_aug)))
            with torch.no_grad():
